...
## 13. 发送消息后空白无反馈、等待很久才看到思考过程

**症状**：Web 端点击发送消息后，需要等待数秒（模型 Time-To-First-Token 期间），界面完全空白静止，给人“卡住/响应极慢”的感觉；且思考过程开始流式返回时，默认折叠导致看不见动态文本。

**根因**：
1. **空白等待期**：前端在 `submitPrompt` 或 `agent_start` 收到时，未创建助手消息气泡与加载指示器，而是静默等待 LLM 返回第一个 chunk（`thinking_delta` 或 `message_start`）才创建节点。LLM 请求网络往返+Prefill耗时 2-5s 期间，界面没有任何视觉反馈。
2. **思考内容隐藏**：`makeThinkingBlock` 默认设置 `body.style.display = "none"`。流式生成思考内容时，思考框处于折叠状态，用户只看到静态的 `💭 思考过程` 标题，看不到实时思考文字，误以为思考未开始。

**修复**：
1. **即时渲染占位符**：点击发送/收到 `agent_start` 时，立刻创建助手气泡并展示动态 `[ Spinner ] 正在思考中…` 占位框，提供 0ms 视觉响应。
2. **思考过程实时展开与高亮**：在思考流式生成期间，思考框自动展开并带有呼吸灯高亮（`💭 正在思考中…`），让用户直观看到模型的实时推导过程；生成结束后自动收起/保持折叠。
